...
> **2026-09-04 交互视觉稿（原型模块 v2.2 / UI 设计 v2.1，2026-09-05）**：本方案 §5/§6/§7 的可点击视觉稿按模块分稿，落 [designs/screen-prototype-design.html](./designs/screen-prototype-design.html)（原型设计模块 v2.2）与 [designs/screen-ui-design.html](./designs/screen-ui-design.html)（UI 设计模块 v2.1），均为单文件 HTML 零依赖，两模块交互逻辑一致。范式沿用 review-module 工作区主题与 Aqua 双板。**原型模块 v2 结构（用户拍板）**：① 去掉 Hub 图标列；② 模块内三段页签 **需求文档 → 交互原型 → 验收报告**；③ **版本一份、三段同步——一个版本 = 需求文档 + 交互原型 + 验收报告 一套**（切版本三段联动，旧版本只读可回最新，版本集徽章标注缺失段）；④ 左侧面板为**纯目录分组零操作**（工作区分组 + 产物概况，操作全在 sheet 内）；⑤ 全链路交互：一句话 → spec-writer 展开 7 节（流式）→ 确认需求 → 按「页面清单」逐页具现化（根先行流式）→ 交互回路（action → design:action → 语义 ID diff）→ 验收走查（合成 action 序列 + 自愈 ≤3 轮 → meta.verification）→ 报告内「✦ 修复此项 / 一键修复全部」→ **悬浮设计智能体**（跨三段常驻、页签感知快捷项、输入直接下达修复，silent 零会话记录）。**v2.1 选区悬浮修正**：交互原型段点击任意元素 → 设计智能体修正卡**跟随选中位置**弹出（语义定位 + 类型感知快捷项 + action 可直接执行 + 定向输入，Esc/点空白取消；action 执行改经卡内「▶ 执行」，呈现逻辑同构索引/关系图选区悬浮卡；深链 `#proto-sel` 直达演示态）。**UI 设计模块 v2（交互逻辑与原型一致，内容基座保留）**：三段页签 **视觉稿 → 设计系统 → 质量与评审**；**一版一套且每版记录「基于原型 vX」徽章——UI 与原型版本精确联动**；**基底选择 = 原型的版本历史**（原型 v3·走查修复 / v2 / v1）；**「主题」= 设计模块专属九套内置设计系统**（现代极简 / 编辑风 / 暗色科技 / 粗野对比 / 瑞士国际 / 终端等宽 / 玻璃拟态 / 软拟物 / 暖手作，切换确定性重染画布；非产品应用主题）；section 级选区悬浮修正；lint findings 审查报告同款形态 + 「定位 ◎」→ 视觉稿 section 闪烁（双向定位）；机检实测 + review 门；漂移闸门收进视觉稿工具行；悬浮智能体跨三段（页签感知快捷项 + 输入）。深链：原型稿 `#proto` / `#report` / `#proto-sel`，设计稿 `#tokens` / `#quality` / `#pages-sel`。**v2.2 生成/触发显式化**：原型稿——需求文档段常驻「需求输入卡」（一句话 → 流式展开 7 节 → 「待确认」逐条确认清零即定稿）+ 验收走查双触发入口（交互原型工具条「▶ 运行验收走查」跳段自动开跑 + 报告页）+ 触发链路卡；UI 稿——设计系统页签双栏布局（tokens 表 + 组件原子库并排，修复空区）+「⚡ 从原型生成」（读基底原型页面清单 → .dds tokens → 逐页渲染 → 新版本入列）+「▶ 运行质量评审」（lint 三 preset → 机检实测 → review 门 R1→自愈→R2）触发链路卡。预览截图 ×6：preview-prototype-workbench / preview-prototype-report / preview-prototype-selpop / preview-ui-design-workbench / preview-ui-design-selpop / preview-ui-design-tokens。
...
> 现状核实：2026-09-01 直读 `packages/desktop/src/main/tools/dd-package.ts`（226 行）+ `specs/archive/deep-design/design.md` §0 + `specs/design-systems-advance/pm-design-v2-design.md` 头部（格式拍板 2026-08-18，P4-1 已实现）。
