...
- 旧产物兼容（EARS 15/17）回归；guard 测试更新（design.ts 走 render_leafer 禁 render_openui / prototype.ts 零 leafer）；`npm run check && npm test` 全绿；真机走查：一句话生成 → 画布微调 → 版本轨 → lint/review → `.ddu` 双击可交互。
...
| #   | 改动                                                                                                                                                                                                                  | 落点                                                   |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------ |
| 5.1 | **契约强化（M3E #1/#3/#5/#9/#10）**：CREATE 契约新增稳定 `name` 命名指代（root 直接子元素与 Frame 必名）、负向禁令三连（禁叠放重复/禁出画布/禁造色）、自检完成定义句                                                  | `core/src/actions/leafer-contract.ts`                  |
| 5.2 | **确定性 UI→大纲编译器（M3E #2 几何→语义）**：`describeLeaferDocument` 把场景 JSON 编译为语义大纲（画布尺寸/元素按稳定名指代/坐标→方位词/flow 布局短语/文本预览），纯函数、固定遍历序、键序不敏感——同输入字节级同输出 | `core/src/actions/leafer-describe.ts`                  |
| 5.3 | **自检 gate 双源化（EARS 18）**：修复环判定从"结构合法"升级为"结构 ∪ error 级 lint（越界/空场景）"；warning/info 不进 gate（防振荡）；成功值规范化（`JSON.stringify(JSON.parse())`）——落盘/提交/投影三处零格式抖动    | `core/src/actions/leafer-repair.ts` + `leafer-lint.ts` |
| 5.4 | **修订 prompt 无抖动基线（EARS 19，M3E #7 意图直通）**：revise leafer 分支的 prompt = 契约 + 逐字指令 + `describe` 语义大纲 + 规范化 JSON——修订是纯函数式的"读状态→改状态"，不依赖模型对上轮输出的记忆                | `core/src/actions/design.ts`                           |
| 5.5 | **持久化自检（EARS 20）**：`render_leafer` 落盘时规范化文档字符串 + 零 LLM 自动运行确定性 lint 写入 `quality.lintFindings`——质量页永不空窗，review 状态不受影响                                                       | `desktop/src/main/tools/a2ui/a2ui-mcp.ts`              |
