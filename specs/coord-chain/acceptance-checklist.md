...
> 日期：2026-09-04 · 状态：**单机实机部分已执行（本会话）；双机局域网子项（C1/C5-mDNS/C7 真机）为
> 「环境依赖、未验证」**——本会话无第二台设备，三次探针实证单机 mDNS 组播不可达；runbook 已备，
> 双机结果回填处见下文。
...
> 说明：本会话三次探针（① 默认组播 ② 显式 loopback/reuseAddr ③ 双独立进程 + 真实 Discovery 类）
> 全部实证 macOS 本机组播不可达（lo 不转发、Wi-Fi AP 组播隔离）——**mDNS 发现率（C1/C4/C5-mDNS 段/
> C6 首连）属真实双机环境动作**，单机无法替代；协议层其余动作见下述等价覆盖矩阵。

## C 表执行状态矩阵（2026-09-04 会话更新）

| 项                                           | 单机等价覆盖（本会话已执行，同一协议栈）                                                      | 双机真机状态                                       |
| -------------------------------------------- | --------------------------------------------------------------------------------------------- | -------------------------------------------------- |
| C2 资产/任务互通                             | ✅ 双逻辑节点 ws：资产发布→manifest→分块拉取→重组全等；task.share 双向封块（e2e）             | 等价动作已验证；真机指 **C1 发现成功后的同一路径** |
| C3 设备轮换                                  | ✅ 双逻辑节点：rotate → 成员表迁移新钥 → 新钥续签出块；service 重启恢复同钥（e2e）            | 同上                                               |
| C4 跨主题隔离负例                            | ✅ 握手主题钉扎拒连（e2e）+ discovery 过滤纯逻辑单测                                          | 真机补测（改主题重启两台互不可见）                 |
| C5 邀请码兜底                                | ✅ **本会话新增**：`deeporca-chain://` 生成→解析→B 经 ws 入链→成员互见→记录互通（invite-e2e） | 真机补测（防火墙弹窗 + 邀请码人工粘贴）            |
| C6 断线重连续传                              | ✅ reconnect e2e（B 停机 → 新实例重连 → (height,hash) 对齐 → 续传）                           | 真机补测（拔网线 30s）                             |
| C1 mDNS 发现率 / C5 首启防火墙 / C7 真机面板 | ❌ 三次探针实证不可达；无第二台设备                                                           | **待双机 runbook（下节）**                         |

## 双机验证 runbook（两台设备、同局域网、允许 UDP 5353）——C1/C5/C7 回填入口

设备 A（macOS）/ 设备 B（Windows）均 checkout `next/coord-chain` 并 `desktop:build`：

```bash
# 设备 A（已建链后）：
#   1. 打开面板 →「开启共享」（git remote 主题）→ 记录链 ID
#   2. 终端：printf '每秒 1 次 PTR 查询，20s' —— 用 mDNS 工具或面板自检
# 设备 B（不同本机路径、同仓库 remote）：
#   1. 打开面板 →「开启共享」→ 观察 ≤30s 内自动发现并加入（成员列表出现 A）
#   2. 记录 C1 五轮成功率：重复 5 次「B 重启共享 → 30s 内互见」✓/✗
# 完成后按矩阵补测：C2 发布资产→B 拉取；C3 A 轮换→B 成员迁移；C4 改主题→互不可见；
# C5 Windows 防火墙弹窗首次放行体验 + 邀请码 deeporca-chain:// 粘贴入链；
# C6 拔网线 30s 恢复；C7 面板端到端（参照第 B 章交互序列）。
# 结论请回填到本文件（替换上表「环境依赖、未验证」标记），并提交 next/coord-chain。
```
