...
- **R40 项目数据根（`.deeporca/` 类比 `.git`）**：the 工作区的 `.deeporca/` 目录 shall 为本项目全部历史数据与模块行为的统一数据根——既有的 settings/skills/AGENTS.md 之外，**sessions（会话）、reviews（审查报告）、deepwiki（知识库）、designs（设计稿）、jobs（索引构建）、prototypes（原型产物）、task-trees（任务树工作区）均已在**；coord-chain 的账本/blob/视图/记录索引 shall 加入同一根（`<workspace>/.deeporca/coordchain/<chainId>/`）；the 设备密钥 shall 为唯一例外（设备级身份，存用户级目录）。the `coordchain/` 子目录 shall gitignore：链数据可随时从链上重放重建，不依赖也不污染代码仓库远端。
