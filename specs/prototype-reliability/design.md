...
**核心主线（user 2026-09-09 定调）：指令遵循——PRD 是唯一事实源。** 两层含义：

1. **平台由 PRD 判定**：目标平台是 PRD 的一等声明；materialize 依据它决定生成哪几端——「三端全生成」只在 PRD 声明多端时发生，移动端产品不浪费 desktop 生成，Web 产品不硬造 mobile 变体（当前 UI 按钮硬编码 `devices:["desktop","mobile","tablet"]`，与 PRD 无任何联动）；
2. **页面逐页映射**：PRD 页面清单与程序 `$page` 集合**逐页机械比对**（靠页面 ID 列建立可机读映射），而不是现在的「标题存在 + 数量近似」。

成功标准（按主线重排）：

1. **PRD 声明目标平台 → materialize 按声明端生成，verify 校验生成端与声明端一致**（WP0）；
2. **PRD 页面清单逐页映射到程序页面**：缺页 failed、多页 warning、导航闭包与死页面检查（WP0+WP2）；
3. `devices` 含多端的 materialize 在真实持久化下**全部落盘**（当前第二端必失败，WP1）；
4. 设备定向修订改的是**该设备的变体**（当前拿桌面版杂交覆盖手机变体，WP1）；
5. verify 能机械证明**无死按钮**（含 `Action([])` 与 bare-string，WP2）；
6. 已生成端随 `.ddp` 导出与投影文件交付（当前只有桌面版，WP4）。
...
## 3. 方案 — 五个工作包（WP0 为核心主线）

### WP0 PRD 驱动的平台判定与页面映射（指令遵循主线，先行）

> user 2026-09-09 定调：核心是**指令遵循**——以 PRD 判定真实需要的平台，三端生成是 PRD 声明的结果而非默认行为。

| #   | 改动                                                                                                                                                                                                                                                                                                                       | 文件                                                      | 测试                                                                       |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------- | -------------------------------------------------------------------------- |
| 0.1 | **spec-writer 契约升级**：信息表加「目标平台」必填行（`web / mobile / tablet / desktop-app / mini-program / 多端组合`，从需求推断；推断不出 → 待确认项），非功能需求节的平台行同步升级为一等声明；**页面清单表加「页面 ID」列**（英文 kebab-case id，即程序 `$page` 值，如 `订单列表(orders)`），建立 PRD↔程序的可机读映射 | spec-writer/SKILL.md 模板                                 | spec 契约测试：生成的 PRD 含目标平台行 + 页面 ID 列                        |
| 0.2 | **core 解析**：`openui-pages.ts` 增加 `extractTargetPlatforms(spec)`（解析目标平台声明 → 设备集合，含「多端(web+mobile)」组合式写法）与页面 ID 列解析；旧格式 PRD（无声明/无 ID 列）返回 null 走降级路径                                                                                                                   | 新模块（WP2.1 同文件）                                    | 单测：组合声明/单一声明/旧格式降级                                         |
| 0.3 | **materialize 平台判定**：`devices` 缺省时用 PRD 判定结果（mobile-only 产品只生成 mobile 端）；PRD 亦未声明 → desktop + 「平台未声明」观察项（推动补 PRD）；**UI「生成原型」按钮去掉硬编码三端**，改走判定                                                                                                                 | `prototype.ts:519-524` + `PrototypeWorkspace.tsx:515-524` | core：PRD 声明 mobile → 仅一次 render(device:"mobile")；UI：入参无 devices |
| 0.4 | **verify 平台一致性**：PRD 声明端集合 vs 实际生成端集合（openui 本体 + 变体存在性）——缺端 → failed、多端 → warning 观察项；与页面覆盖检查（2.2，升级为逐页比对）同批落                                                                                                                                                     | `prototype.ts` verify                                     | 声明 mobile 而只生成了 desktop → failed                                    |

**与既有三端机制的关系**：平台变体存储/定向修订（`4afa702c1`）不变——WP0 只改变「生成哪几端」的决策来源（UI 硬编码 → PRD 声明），并补上「生成端必须与声明端一致」的验收闭环。

### WP1 修复多端循环断链（多端生成跑得完）
...
| 2.2 | verify 增三项确定性检查：**导航闭包**（@Set 目标 ∉ $page 值 → failed）、**无死页面**（$page 值未被 root 三元引用 → failed）、**页面覆盖度**——基于 WP0.1 的页面 ID 列**逐页比对**（PRD 页缺失 → failed、程序多页 → warning）；旧格式无 ID 列降级为数量比对。变体每端各跑 | Axure 交互连线 + 页面树理念的机械化；逐页映射依赖 WP0 的 ID 列契约 |
...
- 不动 DSL 定时器原语（属 §4 引擎重规划，A-1 补丁除外）；
- 不做契约生成化防漂移（P2，待 WP2 落地后评估）；
