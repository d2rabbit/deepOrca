...
> **对应实现域**：`packages/desktop/src/renderer/`（ContextMenu 组件 + 各表面接线）与 `packages/desktop/src/main/`（ShellRevealInFolder / EditorFileOps 两个 IPC）。编辑器交互以 [`specs/editor-copilot`](../../archive/editor-copilot/design.md) 为唯一权威（该 spec 已收官归档，其落地实现即权威现状）——本 spec 不替换任何既有编辑器交互，仅在文件树/tab/既有菜单追加。
