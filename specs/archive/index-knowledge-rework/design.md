...
> **✅ 已归档（2026-09-03 用户拍板：全部实现完毕，标记结束）**——R2 任务全量落地（IndexLibraryPanel 工作区重构 / 一键串行构建 / openwiki skills ENOENT 修复 / memory+routing 卡片移除 / wiki+AGENTS.md+架构图可点开等）；tasks.md 已全量勾选。
...
**左侧面板（rail「索引与知识」视图）**——工作区列表，**每行自带构建按钮**：
...
- **按钮追随工作区**：不存在面板级全局按钮——每个工作区条目一个构建按钮，各自独立发起/显示进度（多工作区可并行构建，进度互不覆盖）。
- 条目布局：`[状态点] 工作区名  [上次构建] [构建按钮]`；点击条目主体（非按钮）→ 打开/聚焦右侧知识 tab；点按钮 → 只构建，不切换 tab。
- 行内空间不足时按钮收为图标（⟳/▶），悬停提示完整「构建索引与知识」。
...
- 左侧**只展示工作区**（不展示 codegraph/openwiki/agents.md 等源概念）；每个工作区条目带汇总状态（索引是否就绪/上次构建时间）与**自己的构建按钮**。
- **tab 生命周期**：主 tab（会话）常驻；索引与知识 tab 每工作区至多一个，点击左侧条目主体打开/聚焦，× 关闭；切换工作区时 tab 内容跟随或复用同工作区的既有 tab。
- 构建按钮作用于**所在行的工作区**（非"选中项"），进行中该行显示分段进度，完成后刷新该行状态与对应 tab 内容；多行构建互不阻塞。
...
- **B1 一键串行（每工作区一个）**：「构建索引与知识」按钮位于每个工作区条目行内，作用于该行工作区（`index.build-all` 单入口），严格串行 符号索引 → Wiki → 架构图（现状编排顺序正确，保留）；按钮态：进行中显示分段进度 `[2/3] Wiki…`（**进度文案禁用引擎名**，见命名红线），任一段失败即停并显示错误（后续段跳过，标 skipped）。
