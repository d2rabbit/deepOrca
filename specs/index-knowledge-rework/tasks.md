...
- [ ] T3.1 左侧 `IndexLibraryPanel` 重构：**只列工作区**，每行 = 状态点 + 工作区名 + 上次构建 + **行内「构建索引与知识」按钮**（作用于该行工作区；`index.build-all` 串行 符号→Wiki→图，失败即停显示错误；多行构建独立互不阻塞；行内空间不足收为图标+悬停提示）；点条目主体 → 打开右侧知识 tab，点按钮 → 只构建不切 tab
