...
- [ ] T3.1 `IndexLibraryPanel` 重构：单工作区卡片（路径/汇总状态/上次构建）+「构建索引与知识」唯一主按钮（`index.build-all`，串行 符号→Wiki→图，失败即停显示错误）
- [ ] T3.2 产出物列表替代六源卡片：📊 符号索引（状态+重建二级）/ 📚 Wiki（页面列表，点开→编辑器）/ 📋 AGENTS.md（点开→编辑器）/ 🏗 架构图（列表，点开→右侧预览 dock，复用 onShowGraph 单槽）
- [ ] T3.3 移除 memory / routing / serena 卡片及其内嵌 UI（L0-L3 分解、语义搜索框）
...
- [ ] T5.2 AGENTS.md 单条目 + 点击打开
- [ ] T5.3 架构图 HTML 文件枚举 + 点击接右侧预览 dock（与 CRG 图互斥单槽）
