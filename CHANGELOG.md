...
### 🏗️ 索引与知识模块重构（2026-08-29/30）

- **架构图管线整体切换到 [Archify](https://github.com/tt-a1i/archify)**（vendored `v2.15.0`，git 钉版 + 原子交换 + 完整性守卫）：后台 LLM 任务作者 typed-IR（`arch-*.<type>.json`），宿主跑确定性交付门禁（schema/布局/渲染校验 + 原子提交 + sha256 回执）；自有 Mermaid 板子/A2UI 面板/save_archmap 全链退役。知识面板架构图 tab = 内嵌画板（hero iframe `?present=1` 直出铺满 + 子级 ArchifyMiniMap 动态 SVG 绘制，索引关系图交互语法）+ 独立窗口（单实例/追踪主窗口生命周期）。
- **Wiki 存储事务化（deepwiki）**：`openwiki/` 仅为生成临时舞台，CLI 从不触碰 `deepwiki/`；update = 拷贝→增量→验证→swap-aside promote（原子）→清扫；好库在任何坏运行下不可损。交付回执（sha256 sidecar）+ 451 网关审查自动重试（`[hint:model-censored]` 六语言）+ StepFun Responses-API 方言根因修复（openai-compatible provider 路由）。
- **后台任务安全加固**：`write` 入面 + 路径授权（目标根 prototypes 只写）；bash 变更命令执行前拦截（静态分析 + 裸 validate 白名单）+ 一等 `validate_archifact` 工具引导；工具描述定向改写（bash=read-only / write=primary）；产物检查点回滚（坏运行结构上不可毁好图）；网关审查类快速路径（无变更不邀请 LLM）；git 预检/引导（子目录守卫 + .gitignore 播种 + 服务端复核）。
- **CodeGraph SDK 化**：`recreate()` 替代 init-已存在崩溃、`close()` 替代 `uninitialize()` 误删、hasProject=初始化且节点>0、懒加载 node:sqlite、符号计数直读。三阶段构建管线链式失败即停 + 空产物内容量守卫（wiki 512B / arch 256B / codegraph 0 符号）。
- **架构图语言跟随应用 locale**（`configureArchifyLanguage` 注入缝，与 wiki `--language` 同源）。
- **GVGL 首建架构图交付门失败处置（实机 2026-08-30）**：模型产出的 IR 布局不合法（作者端 `fromSide/toSide` 与几何矛盾、枢纽跨板连线 5 处交叉、标签压盖组件/互相 0px）→ 交付门按契约拒绝，且该 IR 事后消失（疑似增量部分重写碎片被内容清扫吞掉）。处置：① 按 DESIGN.md 事实手工重建合格 IR（8 组件 5+3 网格相邻布局、双向对共享泳道 + 两侧 `labelAt` 锚定、3 叙事章节 + 3 结论卡 + python 品牌标记），deliver **9/9、0 错误 0 警告**，宿主补丁/回执落盘，面板即刻可见；② skill 固化四条教训：枢纽组件邻居就地放置（跨板长边=交叉工厂）、`fromSide/toSide` 属诊断后才可用的方向契约、双向对共享泳道合法而镜像 `via` 会禁用端口展开（修标签不修路线）、最终字节必须以通过的 validate 收尾（宿主门禁重跑同一套检查）。
- **wiki Index 置顶（用户拍板方案落地 2026-08-30）**：wiki 页列表原按文件 slug 排序，每节 `index.md` 被压到中后段（client/：g-slug 在前；gvgllc-core/：卡在 id-stabilizer 与 indexing 之间）。`WikiListPages` 排序后接纯函数 `orderWikiPagesIndexFirst`（`main/wiki-page-order.ts`）：按目录稳定分区，basename `index.md`（大小写不敏感）提升到所在节首位，其余保持 path 序——侧栏树、阅读顺序、上一篇/下一篇 pager 三处消费点同源生效。夹具取自 GVGL 真实结构。
- **架构图引导 rail 还原原生顶部横栏（2026-08-30 用户最终拍板）**：引导目录三轮实验（v1 单行紧凑 → v2 右侧栏 → v3 同格叠放 → v4 悬浮画板左上）全部回退——archify 原生全宽顶部横栏胜出。rail-compact 注入移除，strip 清理逻辑保留，存量图（含各轮实验块）由应用打开面板时经回执自愈自动剥离、回归原生横栏并重钉回执。保留的三块桌面补丁（护照跟随节点/演示锁定/主题活同步）不受影响；实验期间修掉的两个真 bug（theme script 缺闭合行、active-view=all 状态规则压盖）保持已修状态。
- **回执 HMAC 回退键缺陷修复（GVGL 实机暴露）**：非 Electron 上下文（测试/裸 node）的 HMAC 密钥原为进程随机——每次运行换钥匙，跨进程回执必失配（GVGL 刚钉的回执下个进程即失效，rail 补丁清扫跳过文件）。回退改为**确定性派生键**（真实安装仍用 userData 持久密钥，安全边界不变），跨进程验证测试通过。
- **红蓝对抗审查（5 路并行 agent：3 红攻击 / 2 蓝验证）落地修复（2026-08-30）**：
  - **安全（红·HIGH）**：① 交付回执可被模型伪造（回执 sidecar 就在模型可写的 prototypes 目录里，明文 sha256 可自算）→ 回执升级为 **HMAC 签名**（`htmlPath|sha256` 绑定 + userData 每安装密钥，timingSafe 比较；旧回执自动失效一次重渲染完成迁移）；② `KnowledgeOpenArchHtml` 预览窗**从不校验回执**，是同一不变量的绕过入口 → 补上回执门（非宿主交付渲染直接拒绝）；③ `runArchifyValidate` 的绝对路径分支未 resolve，`../..` 可穿过前缀检查 → `path.resolve` 后再判。另：补丁写盘改 temp+rename 原子化（file:// 读者并发加载不再见截断文件）、预览窗 destroyed-close / 陈旧 closed 处理器守卫、postMessage 增 iframe onLoad 重发（加载中途切换主题的丢失窗口）。
  - **状态机（红·MEDIUM）**：`record()` 无 failed/skipped 终态迁移——"[1/3] CodeGraph stage failed" 后下一个 "[2/3]" 头把失败段升格成绿色 done、被跳过的 wiki 打出绿色 "wiki done" → 动作侧 verdict 消息规范化（"stage failed"/"stage skipped"），manager 侧解析终态且隐式晋升不再触碰终态段。另：阶段失败诊断不再截断（原 200 字符正好截掉目录清单与 lastValidate 证据）、prototypes 清单带文件大小（≤256B 不可见规则可解码）、检查点回滚从"全有全无"改为逐件恢复。
  - **契约（红·HIGH）**：SKILL 的 Showcase 段以架构图一型之词覆盖五型——`layout`/`boundaries` 仅 architecture 有、sequence participants 无 `tag`、lifecycle states 用独立 type 枚举、workflow lanes/phases/groups 与 sequence `return` 的 variant 合法性 → 逐条 scoping 改写；brands/guide 查询被 bash 白名单拦截（品牌标记功能在后台运行时必死）→ 白名单扩展 + bash 工具描述同步；`meta.views`/`cards` 的 "2–5/1–3" 标注为风格建议而非 schema 边界；补 "一次诊断一个几何控制" 与 "9 检查 0 警告" 的验收表述；提示词 Showcase quality bar 同步 scoping。
  - **蓝军独立复测**：四工作区 1083 测试全绿、真实 CLI deliver 9/9、补丁幂等（字节级）、伪造场景（stdout 注入/meta.output 重定向/品牌 SSRF/原型污染等 12 向）全部失败、GVGL 旧回执经宿主流程一次迁移即绿。
- **新项目首次构建索引崩溃修复（GVGL 实机 2026-08-30）**：CodeGraph 适配器的 reindex 对**从未初始化**的工作区也无条件走 SDK `recreate()`，而 SDK 的 recreate 只接受已初始化项目（"Run init() first" 是它的硬守卫）——新工作区首建 1/3 阶段必炸、整个构建 FAILED。修复 = 按 `isInitialized` 分派（新根 → `init()`，已初始化/空心残留 → `recreate()`，两者本就是互补对）；真实 SDK 集成测试钉住两个分支。附带修复：阶段失败时 index-build 仍打印 "[1/3] CodeGraph done" 的误导性日志改为 "stage failed"（成功路径消息不变）。二连构建确认修复实机生效（`codegraph=true`）。
- **arch-scan"绿校验/空门禁"失配加固（GVGL 二连实机 2026-08-30）**：模型声称"已产出并校验"而交付门找不到产物、且 `validate_archifact` 回执为绿的失配不可解码。三处加固：① `runArchifyValidate` 路径契约从 `startsWith(目标根)` 收紧为**必须位于 `<root>/.deeporca/prototypes/` 且文件名精确匹配 typed-IR 模式**（与交付门同一目录契约——绿校验从此只可能落在门禁读得到的文件上）；② 任务结果携带 `lastValidate`（最后一次校验的路径+结论），阶段失败消息附上 prototypes 目录清单与该路径，失配从日志即可解码；③ "nothing to render" 消息区分"全部为当前版本（回执通过）"与"确实未产出"两种语义（前者曾把空心结果掩盖成正常）。
- **架构图查看器三块桌面补丁**（交付后注入，幂等收敛 + receipt 重钉，存量图打开面板即原地升级免重渲染）：① **语义护照跟随节点**（v2 重写——v1 依赖模板从不设置的 `data-node-id` 属性静默失效；v2 改读查看器真实状态 `data-focus-selected`/`svg[data-focus-active]`，右侧优先、边缘翻侧、垂直居中、平移缩放滚动实时跟随，含 wide-diagram 滚动坐标换算）；② **演示舞台锁定**（Present/Exit 能力整体移除：按钮隐藏 + 捕获阶段吞点击 + `Archify.presentation` API 出口重绑 + `data-present` MutationObserver 守住，F/Escape/guide 入口全部废掉，始终保持放大态）；③ **主题活同步**（`{type:"deeporca-theme"}` postMessage 严格校验通道，画板跟随应用明暗切换且不重载 iframe——初始 `?theme=` 与活同步结合，保留读者缩放状态）。
- **arch-scan authoring skill 升级到完整 showcase 表现面**（"更完善、更有美感"拍板 2026-08-30）：精确语义类型定配色、每组件 sublabel + 运行时 tag、emphasis/security/dashed 变体、真实归属边界（region/security-group）、1–3 张证据结论卡、2–5 个 `meta.views` 叙事章节（故事轨道/跟随镜头）、`brands` CLI 品牌图标、grid 放置（0 起算/格唯一/低价值边先删的实测教训内嵌）；任务提示词同步带 Showcase quality bar（fresh + incremental）；底线 = 无证据的美感特征即造假。真实管线端到端验证：完整表现面 IR 过 showcase 修复循环后 deliver **9/9 checks、0 错误 0 警告**，边界/章节/卡片/品牌标记确认真实渲染。
