...
`a2ui-mcp.ts` 在同一个服务器上注册 **12 个工具**（`registerDesignTools` 追加 .dd 工具；R2 起 `navigate_to` 移除、`save_archmap` 加入，后增 `render_spec`）：
...
| 规格渲染 | `render_spec` | 需求/规格文档 → 渲染视图 |
| 架构图持久化 | `save_archmap` | 把架构图写成 **Mermaid 文档**（`.deeporca/prototypes/arch-<name>.md`，完整替换；**`format`/`html` 参数与 `arch-*.html` 分层板形式 2026-08-28 退役**——一图一形）；文档布局 = 标题 + 一句话定位 + 每视角一个 ```mermaid 围栏 + **「架构分析」证据驱动评审章节 + 「优化建议」问题→建议→优先级章节**（2026-08-26 f57cb6fb 增补）；Knowledge 面板把每个 ```mermaid 围栏渲染成真实图 |
