...
- Tool discovery: caches `tools/list`; `tools/list` change notifications refresh `mcpToolDefinitions` (visible to the session manager). `refreshServerTools` carries a **per-server mutex**（b316f3a7）: concurrent `tools/list-changed` notifications (or a refresh racing a change notification) drop later runs instead of double-registering the same tool names — duplicate registrations made the OpenAI endpoint reject the whole request with a 400.
