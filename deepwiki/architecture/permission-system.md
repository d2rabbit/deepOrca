...
- **bash**: the tool schema requires the model to declare a `sideEffects` array (`read-in-cwd`, `write-in-cwd`, `delete-in-cwd`, `network`, `mutate-git-log`, etc.); unknown side effects → `unknown` scope. **推断保守化**（2026-08-27，commit e9578124）：`cp`/`mv`/`install`/`ln`/`touch`/`patch`/`rsync`/`sed -i` 等写原语、`mv` 的删除面、以及解释器执行仓库内脚本（按扩展名锚定、大小写不敏感）**不再被分类为只读自动执行**——未声明的写副作用现在按 `unknown`/写作用域走权限门，而不是静默放行。
...
- **首开 fail-closed**（P0 加固，2026-08-27，commit e9578124）：`effectiveWorkspaceTrust` 把**未作答信任对话框**的工作区解析为 `quarantine`（旧默认是 trusted，commit e6e5140a F2 被有意推翻）；`resolveSettingsSources` 随之夹紧项目级执行面（env/endpoints/memory/webSearchTool 等，见 [core/settings](../core/settings.md)）——恶意仓库在信任决定前无法拉起 MCP 或改路由用户凭据。
