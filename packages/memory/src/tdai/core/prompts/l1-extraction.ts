...
4. 时间保真：content 与 metadata 中的时间必须忠于源消息的精度——源消息说"2025年3月"就写"2025年3月"，不得补成"2025-03-01"或"2025年3月1日"；"最近/某天/上个月"等模糊表达保持模糊。例外：消息自带的 timestamp 属于源数据，按任务二的规则从 timestamp 推算绝对时间是允许的（且精度不得超出 timestamp 本身）；推算不出来的精度宁可完全省略，也不要虚构。
5. 原子但有叙事：一条记忆围绕一件事——同一条因果/关联链按规则3合并；但互相独立的事实断言必须拆成多条记忆，不得把不相关的信息塞进同一句。
6. 上下文分界：【背景对话】与【上一个情境】仅用于解析指代与判断情境切换——**严禁把其中已确立的事实当作新消息的产出重新抽取**；增量处理同一会话时，绝不重复产出上文已提取过的记忆。
7. 输出前终检（final sweep）：输出 JSON 前逐条重读每条记忆，专查两件事——① 漏标或错标的时间表达（按规则 4 校正或删除）；② 是否违反规则 6（从背景对话提取）。发现即修正后再输出，校验发生在输出之前，不是输出之后。
8. 逐字保留：具体名目必须逐字保留、不得概括改写——书名/作品名、品牌/产品/项目名、人名/地名/机构名、物种种名、数字与序数（如"三家公司""第 2 次"）、频率表达（"每周二和周四"不得改写成"每周两次"）。可检索性是记忆提取的第一约束：泛化即丢失未来的召回。
