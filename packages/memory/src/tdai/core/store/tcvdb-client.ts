...
export class TcvdbClient {
  ...
  constructor(config: TcvdbClientConfig, logger?: StoreLogger) {
    ...
    this.logger?.debug?.(
      `${TAG} url=${this.baseUrl} db=${this.database} timeout=${this.timeout}${this.baseUrl.startsWith("https://") ? ` https=true caPemPath=${config.caPemPath ?? "(none)"}` : ""}`
    );
    ...
    if (this.baseUrl.startsWith("https://") && config.caPemPath) {
      try {
        ...
        this.logger?.error(
          `${TAG} Failed to load CA PEM from ${config.caPemPath}: ${err instanceof Error ? err.message : String(err)}`
        );
      }
    }
  }
  ...
  async request<T = ApiResponse>(path: string, body: Record<string, unknown>): Promise<T> {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      ...
      try {
        ...
        this.logger?.debug?.(
          `${TAG} ← ${path} status=${statusCode} code=${json.code} attemptMs=${attemptMs} attempt=${attempt}`
        );
        ...
        if (attempt < MAX_RETRIES) {
          ...
          this.logger?.debug?.(
            `${TAG} ${path} retry ${attempt + 1}/${MAX_RETRIES} in ${delay}ms (lastAttemptMs=${attemptMs}, error=${lastError.message})`
          );
