...
export class TcvdbMemoryStore implements IMemoryStore {
   ...
   */
  async searchL1HybridAsync(params: { queryText: string; topK?: number }): Promise<L1SearchResult[]> {
  ...
  async queryL0ForL1(sessionKey: string, afterRecordedAtMs?: number, limit = 50): Promise<L0QueryRow[]> {
    try {
      ...
      const docs = await this._queryAllDocs(this.l0Collection, filterExpr, L0_OUTPUT_FIELDS, limit, [
   ...
   */
  async searchL0HybridAsync(params: { queryText: string; topK?: number }): Promise<L0SearchResult[]> {
    const { queryText, topK = 10 } = params;
    if (!queryText) return [];
  ...
  async syncProfiles(records: ProfileSyncRecord[]): Promise<void> {
    ...
    try {
      ...
      const remoteMap = new Map(remoteDocs.map((doc) => [String(doc.id ?? ""), doc] as const));
      const now = Date.now();
