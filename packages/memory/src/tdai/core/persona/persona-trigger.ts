...
export class PersonaTrigger {
  ...
  async shouldGenerate(): Promise<TriggerResult> {
    ...
    this.logger?.debug?.(
      `${TAG} Evaluating: total_processed=${cp.total_processed}, last_persona_at=${cp.last_persona_at}, memories_since=${cp.memories_since_last_persona}, scenes=${cp.scenes_processed}`
    );
    ...
    if (cp.scenes_processed > 0 && cp.last_persona_at === 0 && (await this.hasSceneFiles())) {
      ...
      this.logger?.debug?.(
        `${TAG} Trigger P2 (cold start): scenes_processed=${cp.scenes_processed}, total_processed=${cp.total_processed}`
      );
      return result;
    }
    ...
    if (cp.last_persona_at > 0 && (await this.hasSceneFiles()) && !(await this.hasPersonaBody())) {
      ...
      this.logger?.debug?.(
        `${TAG} Trigger P2.5 (recovery): last_persona_at=${cp.last_persona_at}, persona body missing`
      );
      return result;
    }
