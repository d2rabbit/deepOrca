...
export class SceneExtractor {
  ...
  async extract(memories: Array<{ content: string; created_at: string; id?: string }>): Promise<ExtractionResult> {
    ...
    try {
      ...
      this.logger?.warn(
        `${TAG} extract() failed to update persona navigation: ${navErr instanceof Error ? navErr.message : String(navErr)}`
      );
    }
    ...
    if (this.instanceId && this.logger) {
      ...
      const resultScenes: Array<{ title: string; summary: string; content: string; status: "created" | "updated" }> =
        [];
      let scenesCreated = 0;
      let scenesUpdated = 0;
      let scenesDeleted = 0;
   ...
   */
  private buildSceneSummaries(index: SceneIndexEntry[]): { summaries: string; filenames: string[] } {
  ...
  private async updateSceneNavigation(): Promise<void> {
    ...
    if (!stripped) {
      this.logger?.debug?.(
        `${TAG} updateSceneNavigation() skipped: persona body is empty, waiting for PersonaGenerator`
      );
      return;
    }
