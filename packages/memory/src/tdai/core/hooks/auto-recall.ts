...
async function searchMemories(
  ...
  try {
    if (effectiveStrategy === "keyword") {
      ...
      return {
        lines,
        timing: { ftsMs: performance.now() - tFts, embeddingMs: 0, ftsHits: lines.length, embeddingHits: 0 },
      };
    }
    ...
    if (effectiveStrategy === "embedding") {
      ...
      return {
        lines,
        timing: { ftsMs: 0, embeddingMs: performance.now() - tEmb, ftsHits: 0, embeddingHits: lines.length },
      };
    }
...
function applyRecallBudget(lines: string[], recall: MemoryTdaiConfig["recall"], logger?: Logger): string[] {
