...
export class BackupManager {
   ...
   */
  async backupFile(srcFile: string, category: string, tag: string, maxKeep: number): Promise<void> {
    try {
   ...
   */
  async backupDirectory(srcDir: string, category: string, tag: string, maxKeep: number): Promise<void> {
   ...
   */
  async restoreLatestDirectory(category: string, destDir: string): Promise<{ restored: boolean; from?: string }> {
 ...
 */
async function pruneOldEntries(dir: string, maxKeep: number, kind: "file" | "directory"): Promise<void> {
  let entries: string[];
  try {
