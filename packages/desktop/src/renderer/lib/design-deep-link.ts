 ...
 *       `#prototype` / `#prototype/spec|proto|report|arch`.
 */
export type DesignWorkspaceTabKind = "prototype" | "design";

export type DesignWorkspaceTabSegment = "pages" | "tokens" | "quality" | "spec" | "proto" | "report" | "arch";

export function parseDesignHash(
  hash: string
): { kind: DesignWorkspaceTabKind; tab?: DesignWorkspaceTabSegment } | null {
  const match = hash.trim().match(/^#\/?(design|prototype)(?:\/(pages|tokens|quality|spec|proto|report|arch))?$/);
