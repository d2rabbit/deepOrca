...
通过内置的**本地 GitMCP 服务器**让关于**外部 GitHub 仓库**的回答有据可依——每个注册的仓库都以本地 stdio MCP 服务器运行，共享一个磁盘索引库（`~/.deeporca/gitmcp/index.db`）。文档只从 GitHub 抓取一次，随后在本地分块索引（FTS5/BM25），检索可离线工作，不经过任何第三方服务，消除对小众、新兴或快速迭代库的 API 幻觉。
...
**CLI：**在 `~/.deeporca/settings.json` 中添加占位条目——引擎启动时会自动改写为实际的本地服务器命令：
