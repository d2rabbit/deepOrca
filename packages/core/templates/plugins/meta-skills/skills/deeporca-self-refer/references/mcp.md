# DeepOrca CLI MCP 配置指南

DeepOrca CLI 支持 MCP（Model Context Protocol），让 AI 助手能够连接外部工具和服务，如 GitHub、浏览器、数据库等。
...
配置完成后，启动 `deeporca`，在聊天中输入 `/mcp` 即可查看所有已配置的 MCP 服务器状态以及每个服务器提供的工具列表。
...
3. 运行 `deeporca` 的终端是否有网络访问权限

### 工具不显示

1. 确认 `settings.json` 中的 `mcpServers` 字段格式正确
2. 启动 deeporca 后使用 `/mcp` 查看服务器状态
3. 如果服务器状态显示错误，根据错误信息排查

### Windows 用户

在 Windows 上，DeepOrca CLI 会自动为 `.cmd` 命令添加 shell 支持。如果你的 MCP 命令是批处理脚本，确保文件名以 `.cmd` 结尾。
