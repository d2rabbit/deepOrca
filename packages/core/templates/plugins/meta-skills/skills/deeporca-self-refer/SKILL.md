...
This Skill helps you answer user questions about DeepOrca CLI itself by consulting the reference documentation bundled with this Skill. All docs live in the `references/` subdirectory — always refer to them for authoritative answers.

## When to use this Skill

Use this Skill when the user asks any question about DeepOrca itself, such as:
...
- Any other question about DeepOrca CLI's features, configuration, or usage.
...
- Clearly separate discoverable skills from other concepts:
  ...
  - Bundled skills are discoverable skills shipped with DeepOrca, such as `bundled:deeporca-self-refer/SKILL.md`.
...
- Clarify user-level (`~/.deeporca/settings.json`) vs project-level (`.deeporca/settings.json`)
...
3. **Be specific about file paths** — always specify whether it's `~/.deeporca/settings.json` or `.deeporca/settings.json`.
...
- Skills are discovered from: `./.deeporca/skills/`, `./.agents/skills/`, `~/.deeporca/skills/`, `~/.agents/skills/`, and bundled built-in skills such as `bundled:deeporca-self-refer/SKILL.md`.
- In a source checkout, check `templates/skills/bundled/*/SKILL.md`; in a packaged install, check `dist/bundled/*/SKILL.md`.
- Built-in bundled skills may include `deeporca-self-refer`, `plan`, `skill-digester`, and `skill-writer`; verify the actual list by scanning the bundled root because it can change between versions.
- Use `/skills` slash command in the DeepOrca CLI to list all available skills
...
- Add to `settings.json` (user-level `~/.deeporca/settings.json` or project-level `.deeporca/settings.json`):
...
- After saving, use `/mcp` in DeepOrca to verify the server is running
