...
description: >-
  智能代码审查 — 通过 review.full action 一键编排 CRG 风险分析 + OCR 语义审查。
...
通过 `review.full` action 一键编排 CRG（结构风险分析）和 OCR（AI 语义审查），输出带风险标注的统一审查报告。
...
review.full
  ...
  → ③ OCR 语义审查（带 CRG 风险上下文）
       · ocr review --format json --audience agent --background "<CRG 上下文>"
  → ④ mergeReviewWithCrgRisk → 每条 OCR 评论标注 CRG 风险等级
  → 输出：统一审查报告
```
...
- **OCR 严重程度**：critical / warning / info
- **修复建议**：OCR 生成的具体代码建议

## 注意事项

- CRG 分析需要预先构建图谱（`.code-review-graph/` 存在）。可用 `crg.reindex` 构建。
- OCR 使用 DeepOrca 内置的模型设置，无需额外配置。
- 审查范围默认是未提交的工作区变更。
- 对非代码文件（图片、二进制、锁文件），自动跳过。
