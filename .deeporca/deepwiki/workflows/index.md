...
- [知识构建（CodeGraph → OpenWiki → arch-scan 架构图）](knowledge-build.md) - 一键构建串行流水线：git preflight/bootstrap 前置引导，BuildJobManager 驱动 core action 依次完成 CodeGraph 索引、OpenWiki 文档与架构图扫描（Mermaid 文档），含 3 阶段可观测、首坏阶段即停、后置验证与取消传播。
