...
- 聚焦测试：`session.test.ts`（2026-08 拆分后与 `session-persistence.test.ts`/`session-skills-mcp.test.ts` 三件套覆盖创建/回复/压缩/权限/中断/恢复/持久化，共享 `session-test-utils.ts`）、`tool-handlers.test.ts`、`tool-executor.test.ts`。
