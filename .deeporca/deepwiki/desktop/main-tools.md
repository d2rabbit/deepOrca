...
| `WebFetchProvider` | `web-fetch-provider.ts` | **Offscreen Chromium rendering provider for the built-in WebFetch** (hidden-window rendering + static fallback); 2026-08-27 起 `will-redirect` 先 `preventDefault`、挂起被取消导航的 finish/fail（含 ERR_ABORTED）、**DNS 复检通过后重放**（`assertPublicResolvedHost`，跳数上限 5 与静态路径一致，e9da728f） |
| `GitmcpTools` | `gitmcp/` | GitMCP 8 tools (zread research's four enhancements filed separately, commit bdc6227a) |
| `DembrandtBrowser` | `dembrandt-browser.ts` + `dembrandt-provider-child.ts` | Brand ingestion via CDP（见 [design-system](design-system.md)）。**2026-08-27 安全重构**：CDP 移入独立 Electron 子进程（`dembrandt-provider.cjs`）——随机调试端口（`--remote-debugging-port=0`）+ 私有临时 userData（mkdtemp，退出清理）+ stdout 单行握手 `DEMBRANDT_CDP_READY http://127.0.0.1:<port>` + stdin EOF 随父退出；主进程不再持有任何 CDP 端口（原打包版常驻 9333 固定端口，本机任意进程可 `Runtime.evaluate` 进特权窗口）；`getDembrandtCdpEndpoint` 经 `configureDembrandtCdpEndpointGetter` 注入 core spawn env（`DEMBRANDT_CDP_ENDPOINT`）；12s 就绪窗口（6862dbbc 放宽） |
...
| `A2uiServerBuilder` | `tools/a2ui/a2ui-mcp.ts` | In-process A2UI MCP server (**12 tools**：A2UI surface 6 + `save_archmap`（**仅 Mermaid markdown——`format`/`html` 参数 2026-08-28 退役**）+ `render_spec` + OpenUI Lang 2 + `.dd` 2；`save_archmap` 文档布局 2026-08-26 增补「架构分析/优化建议」评审章节，见 [design-system](design-system.md)）；scoped persistence (`persistSurfaces` idPrefix/stamp + `knownSurfaceIds` sweep guard, see [design-system](design-system.md)) |
