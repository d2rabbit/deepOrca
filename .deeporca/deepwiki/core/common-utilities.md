...
| `file-history.ts` | `GitFileHistory`: session-level lightweight Git repository (undo snapshots, `file-history/.git`); `spawnSync` git 调用带 **10s 硬超时**（b316f3a7——网络盘/杀软卡死 git 不再挂死主线程） |
...
| Module | Responsibility |
| --- | --- |
| `public-url.ts` | `validatePublicHttpUrl` (lexical SSRF defense) + **`assertPublicResolvedHost`**（2026-08-27 新增的 DNS 钉扎层——解析主机名后把全部 A/AAAA 过私网分类器，含 v4-mapped IPv6 双形态；WebFetch 静态路径首跳与逐跳重定向调用，`setPublicUrlDnsLookup` 测试桩保持单测密闭，见 [tools](tools.md)） |
