...
| --- | --- | --- |
| `src/session.ts` (~100 lines, composition root) + `src/session-manager-{base,mcp,skills,persistence,lifecycle,tasks}.ts` + `src/session-{types,usage,stream,mcp-hints,helpers,constants}.ts` | `SessionManager` six-layer inheritance chain (split from one 6266-line file, commit f25770c4) | [Architecture/Session Lifecycle](../architecture/session-lifecycle.md) |
...
| `src/actions/` | `ActionRegistry`/`defineAction` + built-in actions（含 `index.build-all` 三阶段编排与 `wiki-variants` 变体过滤谓词；`wiki.translate` 双语化已于 2026-08-27 移除） | [actions](actions.md) |
...
15. **Model capabilities**: `supportsMultimodal`/`defaultsToThinkingMode`/`resolveModelSpec`/`resolveBackgroundLlm` + `isStepfunBaseUrl`/`endpointQuotaKind`（端点额度探测选择）/`endpointModelFamily`/`FAMILY_MODEL_SUGGESTIONS` + types.
16. **Permissions**: `computeToolCallPermissions`/`buildPermissionToolExecution`/`appendProjectAllowedPaths`, etc. + types.
17. **Actions**: `ActionRegistry`/`defineAction`/`dispatchToolCall`/`configureActionSpawner` + all built-in action definitions and run functions + controller seam + `BackgroundLlmTaskOptions`/`BackgroundLlmTaskResult`（无会话后台任务类型，见 [actions](actions.md)）+ `isWikiVariantFile`（wiki 变体过滤）。
