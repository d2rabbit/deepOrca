...
3. **pm-design-v2 独立 HTML 导出**（`.dd` → 单文件 HTML）：拍板**做**——原 §二"建议做"落定，升入 §一 #9。
4. **skill-routing 目标表 G3 大技能分片召回注入**：拍板**纳入收尾批实施**——推翻此前"建议缓做"评估（缓做理由 book-distill ≤300 行缓解论不再作为延后依据），升入 §一 #10。
5. **task-tree artifact 快照切换**（tasks.md:35）：拍板**纳入收尾批实施**——推翻原"待真实需求再立项"缓期（file-history per-session 语义冲突作为实施时须解决的约束而非缓期理由），升入 §一 #11。
...
| 8   | **独立 HTML 导出**（`.dd` → 单文件 HTML）                                                                                                                 | pm-design-v2（tasks.md:154）          | 🟡 📌 | 2026-08-18 拍板"做"（原"可做"评估落定）；React 代码导出、版本切换 UI 维持不做                                                                       |
| 9   | **G3 大技能分片召回注入**                                                                                                                                 | skill-routing（design.md §一 目标表） | 🟡 📌 | 2026-08-18 拍板纳入收尾批实施（推翻"缓做"）；实施注意勿与代码内已实现的 G3=CompositionalSkill 混淆                                                  |
| 10  | **artifact 快照切换**（file-history 复用）                                                                                                                | task-tree（tasks.md:35）              | 🟡 📌 | 2026-08-18 拍板纳入收尾批实施（推翻缓期）；原登记的 file-history per-session 语义冲突转为实施约束                                                   |
