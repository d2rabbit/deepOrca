...
| # | 操作 | 期望 |
| --- | --- | --- |
| C1 | 开启 gate 后问简单问题（"什么是 X"） | **轻轨**：会话卡出现青色 ⚡快速徽标；响应速度与未开 gate 时一致 |
| C2 | 问复杂多权衡问题（含"方案/权衡/风险/要不要"等词） | **重轨**：会话卡琥珀色 ▤深度徽标 |
...
| # | 操作 | 期望 |
| --- | --- | --- |
| G1 | 六语言切换（设置→语言）后看徽标/进度条/面板 | 快速/深度/阶段名/面板文案全部跟随本地化（zh/zh-tw/zh-hk/ja/ko/en） |
