...
| 文档 | 主题 | 对应模块 | 消费 | 备注 |
| --- | --- | --- | --- | --- |
| [2026-09-17-emg-experience-memory-graph-prestudy.md](./2026-09-17-emg-experience-memory-graph-prestudy.md) | **EMG 经验记忆图**（KDD 2027, arXiv 2607.13884）：失败轨迹 vs 专家轨迹的离线图编辑路径 → 纠错规则记忆（「X 下做 Y 别做 Z」）→ 检索注入一次执行，替代测试时 LLM 反思循环；ALFWorld 未见 SR 60.45% vs 42.54%（4B 模型），节点洞察（任务内规则）为主驱动 | 提案落点（未启动）：`packages/memory/`（repair-rule 象限：L1 事实/SOP/触发器均在「成功经验」侧，EMG 补「失败→修复」缺口）、`specs/review-ing/task-tree-hub/`（fork 谱系天然提供配对轨迹——EMG 硬前提的现成答案） | ⬜ | 纯调研留档，零代码变更。**落地判断**：概念级移植（fork 对 + 序列编辑距离即可，不需要 FGW；LLM 只做措辞，与 memory-audit 三段式同构）；stuck 检测（cmb-next backlog）为天然注入触发点；优雅退化=无信号不产规则；**2026-09-17 已落 spec**：[specs/repair-rule-memory/](../../../specs/repair-rule-memory/design.md)（本周期实施，AGENTS.md 回写裁决见 design §0）；**同日全链落地收官**：P0+P1 代码 + T2.1 真机全链（真实 LLM 措辞/快照/AGENTS.md 谱系回写验证）+ T2.2 门禁——specs/repair-rule-memory tasks 全勾 |
| [2026-09-17-fireworks-tech-graph-prestudy.md](./2026-09-17-fireworks-tech-graph-prestudy.md) | **fireworks-tech-graph 绘图哲学**（MIT, 11.4k★；**明确不引入代码**，只吸收设计哲学）：「Evaluate, don't assert」有界反馈环——确定性检查前置（XML/几何/碰撞）+ PNG 视觉回读后置（检测裁切/重叠）+ 定向修订 ≤2 轮 + 诚实报告（视觉审阅不可用如实标 skipped）；几何组合契约（零交叉/≤2 折/≥40px 间距/标签净空）+ 语义形状/箭头词汇 + 风格化工程契约 fail-closed | 提案落点（未启动）：`specs/arch-map-reinforce/`（active spec，吸收落点）、Archify 门禁（几何契约 2–3 项确定性检查）、vision MCP + 离屏 Chromium（视觉回读执行器，零件全在只差编排） | 🟨 | 纯调研留档，零代码变更。**与机械门哲学独立同源互证**（结构校验半边 Archify 已有；缺的是感知验证半边=视觉回读 + 几何层契约）；不引入清单：Python CLI 栈/SVG 直出/GIF/品牌图标库（与 Archify 职责重叠）；**2026-09-17 已落 spec**：[specs/arch-visual-readback/](../../../specs/arch-visual-readback/design.md)（本周期实施；对 arch-map-reinforce §0.4-4 的对账与三门设计见 design §0/§1）；**同日全链落地收官**：P0+P1 代码 + T2.1 真机两态（真 Chrome/真 vision 判 pass）+ 修订收敛（注入缺陷→2 轮收敛）+ 真机发现 F1–F4 回写 design §0.5——specs/arch-visual-readback tasks 全勾 |
