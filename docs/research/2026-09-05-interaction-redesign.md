...
| 任务树 hub | 节点点击→右侧 timeline；轨迹行为行点击→详情面板（hover 已有） | 无右键；无"复制会话 ID"快捷 |
...
## §4 任务树 hub（历史记录：只读 + 溯源 + 引用）

**设计定调**（用户）：任务树是历史记录。**菜单里没有放弃/删除/改状态**——只有"回看它、引用它、定位它的物证"。

### 4.1 节点行（SESSION/CHAT/审查/设计/构建）

| 菜单项 | 适用 | 行为 |
| --- | --- | --- |
| 打开时间线 | session-tree | `onOpenQuick(kind:timeline)`（=左键） |
| 复制会话 ID | session-tree / session-chat | 剪贴板（溯源锚点） |
| 引用到输入框 | session-chat | 注入该会话摘要+ID（`@会话 …` 提示词缀） |
| 在文件管理器中显示 | session-tree / session-chat | 定位 `<id>.jsonl`（transcript 物证） |
| 在文件管理器中显示报告 | review-report | 定位 `.deeporca/reviews/<id>.json` |
| fork 分支 | session-tree | 既有 fork 表单（fork 是"从历史开新枝"，不改变历史本身——保留） |

fork 语义说明：fork 建**新分支新会话**，被 fork 的历史节点原样不动——符合"历史只读"定调，保留。

### 4.2 轨迹行为行（TraceStepRow，hover/点击→详情已在位）

| 菜单项 | 行为 |
| --- | --- |
| 复制参数 JSON | 剪贴板（`step.arg` 原文） |
| 复制结果摘要 | `step.resultMd`（数据已铺） |
| 在编辑器中打开 | arg 是 `{"file_path":…}` 形态时解析→`handleOpenEditor` |
| 在文件管理器中显示 | 同上解析→reveal |

### 4.3 图表视图（graph canvas）节点

右键节点 = 4.1 同款（按 source.kind 分派）；空白处右键 = 「导出图表 PNG」（P3 占位）。
...
- 会话/模型行右键：「在任务树中打开」（跳 hub 该节点）、「复制会话 ID」。
...
| **P1** | 文件树写操作（新建/重命名/废纸篓，§1.3 IPC）+ tab 右键（关闭其他/全部）+ 任务树节点&轨迹行右键（§4）+ CM6 编辑区 contextmenu handler（§3.3） | 写操作 root-pinned 有测试；删除可从废纸篓恢复 |
