...
- **落地回写（2026-09-04）**：✅ `007d57e8` —— 真值表 5 格测试全绿（session-manager-diagnostics.test.ts 10/10）；mutation-check：破坏 unavailable 处理→5 红还原全绿。落地时另修复存量信封缺陷：extractErrorDiagnostics 此前只认 content 数组，对 manager 信封永远返回 []（回合末诊断自始提取不到错误）——现支持信封 output 解析 + assertDiagnosticsEnvelopeOk 路由失败进 unavailable 腿。
...
- **落地回写（2026-09-04）**：✅ `95e0ac5d` —— tool-handlers.test.ts：代码扩展名 output 追加提示存在、.md/.txt 不存在；回合末诊断桥行为不变（38/38）。
...
- **来源**：调研文档 M8。
- **落地回写（2026-09-04）**：✅ `1e5cdf95` —— aux-llm-contract.test.ts 校验器真值表 + 重试语义（内容级计次/预算耗尽 null/传输级单发）；mutation-check 通过；技能匹配模板化后 18/18 回归。
