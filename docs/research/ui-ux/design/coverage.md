...
|---|---|---|---|
| Rail 19 按钮（`App.tsx:1160-1297`） | P1-1 过载、P0-2 条件显隐、P2-1 emoji 图标 | **重做为左缘模块坞**（v3.2）：纵向单列、SVG 描边图标 + 文字标签、hover 提示快捷键；知识源/插件/审查等模块为列表→详情二级浮层；观测与命令留在右侧控制中心 | 01/01+ |
| TopBar 48px 多职拥挤（`TopBar.tsx`） | 标题栏/模型/状态/token 挤一排 | **目标带 40px 极简**（v3.1：只剩目标名 + 步骤进度）；时长/成本/上下文/通知/主题/设置全部收进**控制中心**（右缘居中小浮窗，⌘⇧O/拉手开合，开工默认展开一次；含 16 格模块宫格 + 指令输入 + 我的指令留痕 + 状态观测流） | 01/01+ |
...
| Composer（`Composer.tsx`） | P1-5 待决时整体禁用 | **控制中心指令输入**（v3.1：底部指示条删除，输入收进 CC 并地址化选中对象；待决不禁用） | 01/01+ |
