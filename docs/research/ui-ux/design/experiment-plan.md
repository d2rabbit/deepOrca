...
**进度（2026-08-20）**：✅ E0 完成 · ✅ E1 完成 · ✅ E2 功能层完成（六主题按决策延后）· ⬜ E3 · ⬜ E4。已交付：`renderer/deck/` 全目录（引擎 hook / 目标带 / 模块坞 / 步骤板 / 待决卡 / Tape / 控制中心 / 13 个真实数据面板）、`lib/layout.ts` 开关与自动回落、设置入口、i18n 六语言、`scripts/check-deck-size.js` 行数门禁、206 个 desktop 测试全绿（含 deck-layout 11 用例）。已知留白：成本仪表无单价源显示 "—"；评审点 #1 的核心路径度量待真机进行；license:check 存在 E0 前已有的 sharp-win32 扫描失败（与本方案无关）。
...
### E2 — 控制中心 + 模块坞接线 + 六主题补齐 ⏳ 功能层已完成，六主题延后

- ✅ 控制中心（仪表四宫格接 usage / 指令留痕 / 状态观测流）；模块坞 13 个入口接真实面板数据（车间墙/文件/变更/进程/知识源/账本/任务树/插件/检查点/审查/资产 + Tape/控制中心；通知/设置/编辑器/快捷键诚实占位待 E3）。
- ⏸ **六主题移植按 2026-08-20 决策延后**（当前仅 Liquid Glass；主题选择 UI 与占位文案已留位）。
- **验收**：评审点 #1——核心路径（提问→批准→看 diff）点击数/耗时对比经典布局，决定是否继续投入。（待真机进行）

### E3 — 浮层栈全模块

- 统一浮层栈 + ⌘K 命令层 + 车间墙/账本/任务树画布等，coverage.md 全绿；六主题移植并入本期。
- **验收**：评审点 #2——按 §6 度量决定转正或下线。

### E4 — 提案增量（可选，来自 orca-deck-v3_change.html）

- `.gi` 语义图标类全量替换 emoji；决策点视觉锚（呼吸脉冲/高亮边框）；独立监控台第二窗（BroadcastChannel→Electron 第二 BrowserWindow，需评估，可砍）。
