...
| **某功能的实现依据** | [../specs/](../specs/)                                                     | 正式实现方案以 spec 为准（design.md + tasks.md）。**已实现/部分实现的 spec 在 design.md 状态行有终判回写（2026-08-18 整顿）；规划中的 spec 一律三层齐备：design.md（预研+设计）+ tasks.md（任务指引，含开工前置条件）**。已收官（✅ 且无未决项）spec 物理归档 [../specs/archive/](../specs/archive/)（原则见其 README，2026-09-02 立） |
...
| 类别                                      | 文档                                                                                                                                                                                      |
| ----------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **功能路线**                              | features/feature-roadmap.md（当前版本路线与现状）、features/next-version-plan.md（下一版本规划：自进化引擎/Studio 基座/远程访问/知识编译/工作区语义检索，2026-08-18 立，2026-09-02 增补主线 E）、features/coord-chain-plan.md（**王牌专属路线 OC · AI 协调工作链，优先级高于 next-version**，2026-08-27 立） |
...
- spec 终判 ✅ 且无未决项后，目录 `git mv` 移入 `specs/archive/`（保留历史，全仓引用同步改写，原则与清单见 specs/archive/README.md）；redesign 唤醒时移回 `specs/`。
