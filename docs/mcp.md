# DeepOrca 桌面客户端 MCP 配置指南

DeepOrca 桌面客户端 支持 MCP（Model Context Protocol），让 AI 助手能够连接外部工具和服务，如 GitHub、浏览器、数据库等。

## 概述

配置 MCP 后，DeepOrca 可以：
...
MCP 工具在 DeepOrca 中的命名格式为 `mcp__<服务名>__<工具名>`，例如 `mcp__github__search_code`。

## 配置 MCP 服务器

编辑 `~/.deeporca/settings.json`，添加 `mcpServers` 字段：
...
| 字段      | 类型     | 必填 | 说明                                                                                                                  |
| --------- | -------- | ---- | --------------------------------------------------------------------------------------------------------------------- |
| `command` | string   | 是   | MCP 服务器的可执行文件路径或命令（如 `npx`、`node`、`python`）。当命令是 `npx` 时，DeepOrca 会自动在参数前补充 `-y`。 |
...
让 DeepOrca 直接操作 GitHub 仓库（搜索代码、管理 Issue/PR、读写文件等）：
...
以下是一个配置了 GitHub 和 Playwright 两个 MCP 服务器的完整 `~/.deeporca/settings.json`：
...
配置完成后，启动 DeepOrca 桌面应用，在聊天中输入 `/mcp` 即可查看所有已配置的 MCP 服务器状态以及每个服务器提供的工具列表。

在对话中直接使用 MCP 工具名称即可调用，例如：

```
帮我搜索 GitHub 上 deeporca-cli 仓库的 issues
```
...
3. DeepOrca 主进程是否有网络访问权限

### 工具不显示

1. 确认 `settings.json` 中的 `mcpServers` 字段格式正确
2. 启动 DeepOrca 桌面应用 后使用 `/mcp` 查看服务器状态
3. 如果服务器状态显示错误，根据错误信息排查

### Windows 用户

在 Windows 上，DeepOrca 桌面客户端 会自动为 `.cmd` 命令添加 shell 支持。如果你的 MCP 命令是批处理脚本，确保文件名以 `.cmd` 结尾。
