...
> 版本：v3.27 · 日期：2026-09-19 · 状态：**预生产冻结期**（dev 集成线 + test 冻结线；新功能一律 `next/*`）
>
> **v3.27 更新（2026-09-19 spec 活跃区处置批·用户拍板）**：① **editor-copilot 收官归档**（✅）：Monaco→CodeMirror 6 内核迁移 / AI 结对画布 / 编辑器智能体数字体 / LSP 裸帧中继 / 任务树集成已随 `feat/modern-ui-redesign` 落地。② **三项冻结归档**（🧊，archive/ 新设冻结区登记）：design-systems-advance（复审修复批中止）/ prototype-reliability（方案定稿未实施，§4 引擎重规划底座随稿保留）/ prompt-doc-chain（实施中 5/18 冻结，pd→pm 更名已生效）。③ **一项转入审查区**（→review-ing）：model-fleet-adaptation（X4 真机验证待排期；leafer/clay 后经代码核实双双改判收官，见 ⑥⑦）。④ §0 终判表补登 prototype-reliability / prompt-doc-chain / leafer-ui-engine / clay-ui-runtime 四行并改写路径；chat-redesign / editor-agent 补登 archive/ 冻结区表。⑤ 活跃区现存 3：coord-chain（OC 本阶段方案）+ spec-graph-adoption（2026-09-18 新立）+ **moonviz-engine-replacement（2026-09-19 立项——原型模块生成栈替换 OpenUI Lang → MoonViz wasm，设计基准 `docs/engine-integration-plan.md` v4.1；上游引擎仍在迭代，P0 电池兼作快照体检，P2 物理替换以收敛 + 全绿为硬门槛）**。⑥ **leafer-ui-engine 改判收官**（✅→archive）：立项残留状态头「方案定稿未实施」经代码核实推翻——WP0–WP5 全落地（契约/修复环/画布微调/可交互 .ddu 导出/自检自修复）+ 两轮审查修复批 + EARS 17 双栈路由修复，真机走查已移交预生产清单（`441206563`），未勾 7 项为后续延伸/上游跟踪；同批顺带清偿移动目录内部相对链接 24 处（归档/审查区层级加深一级）。⑦ **clay-ui-runtime 亦改判收官**（✅→archive）：审查区登记「WP0 kill gate 未跑」不实——WP0 已跑且 **PASS** 放行 WP1（`e6ad4ecf5` 三策略实证），WP0–WP4 全落地 + E2E 实测加固批（`1bc8ed372` 五缺陷 + wrapCJK 全面重写）+ vendor 指纹防漂移（`910a938d7`），唯一未勾项为端到端走查（已移交预生产清单）；勘误回写其 §6。两连改判的共同教训：**spec 状态头/任务勾选可能滞后，处置以提交记录与代码落点为准**——正是 spec-graph-adoption drift detection 要机械化的对象。
>
> **v3.26 更新（2026-09-18 spec 活跃区审查归档批）**：① **8 个收官 spec 物理归档 `specs/archive/`**（git mv，全仓引用同步改写）：design-md-collection / design-stage-gates / prd-theme-layer / artifact-landing / sop-extraction / arch-map-reinforce / repair-rule-memory / arch-visual-readback——证据口径：tasks 全勾 + 落地提交 + 真机验证/审查闭合（repair-rule-memory 与 arch-visual-readback 为 `dedfda064` T2 真机收官批；artifact-landing 状态头「未实现」系过期残留，以 tasks 19/19 + 审查修复批为准）。② **§0 终判表对齐**：sop-extraction / arch-map-reinforce / artifact-landing 🟡→✅ 改指归档路径；repair-rule-memory / arch-visual-readback / design-md-collection / design-stage-gates / prd-theme-layer 五项补登 ✅ 行。③ **specs/README 活跃表对齐**：5 个已归档条目移出活跃行；context-menu-interaction 清出活跃表（物理在 next-version，与储备行重复登记）；prompt-doc-chain 补登活跃行（此前漏登记）；同日新立 spec-graph-adoption（ThinkRail 设计借鉴，`.deeporca/specs/` 前置设计轨，方案稿）。
>
...
| spec | 终判 | 一句话 |
| --- | --- | --- |
| [a2ui-integration](../../specs/archive/a2ui-integration/design.md) | ✅ | 原型模块 + MCP + 自建 processor 渲染器全链路 |
...
| [pm-design-v2](../../specs/archive/design-systems-advance/pm-design-v2-design.md) | ✅ | 存储/Action/面板/预览迭代闭环；.ddp/.ddu 独立导出已落地（2026-08-18 收尾批），版本切换 UI 拍板不做；spec 以工件并入 design-systems-advance |
...
| [model-fleet-adaptation](../../specs/review-ing/model-fleet-adaptation/design.md) | 🟡 | 多模型家族注册表深模块（resolveModelSpec）；G0+S0 落地；**2026-09-17 吸收 ai-sdk-experimental-adapter（X 系列）并二次拍板：原生登记仅保留 deepseek/stepfun，S1–S4 转 X4、后续所有新模型一律依托 X 线接入；同日 X0–X3 全量落地（实验传输通道 + models.dev 数据轨，电池 fixture 全绿），X4 真机验证待排期**；2026-09-19 转入审查区 |
...
| [design-systems-advance](../../specs/archive/design-systems-advance/design.md) | 🟡 | 设计模块进阶唯一方案（2026-09-02 并入 prototype-companion）；suite/version v2 + OpenUI Lang 生成链 + .ddp/.ddu 导出已落地；**2026-09-19 用户拍板冻结归档**（复审修复批中止） |
| [editor-copilot](../../specs/archive/editor-copilot/design.md) | ✅ | 编辑器结对与内核域：Monaco→CodeMirror 6 内核迁移 / AI 结对画布 / 编辑器智能体数字体 / LSP 裸帧中继 / 任务树集成已随 `feat/modern-ui-redesign` 落地（2026-09-19 用户拍板收官归档） |
| [sop-extraction](../../specs/archive/sop-extraction/design.md) | ✅ | SOP 萃取通道（memory-audit P3 延伸）：memory.distill P0+P1+P2 全落地，独立审查 3 项发现闭合，tasks 21/21（2026-09-18 归档） |
| [arch-map-reinforce](../../specs/archive/arch-map-reinforce/design.md) | ✅ | 架构图强化层 v2：P0+P1+P2（R1–R6）代码面落地，真机走查移交预生产清单（2026-09-18 归档） |
| [repair-rule-memory](../../specs/archive/repair-rule-memory/design.md) | ✅ | 修复规则记忆：fork 谱系配对 + 确定性 diff + AskUserQuestion 审查回写 AGENTS.md；P0+P1 + T2 真机验证收官（2026-09-18 `dedfda064`） |
| [arch-visual-readback](../../specs/archive/arch-visual-readback/design.md) | ✅ | 架构图视觉回读闭环：分层三门 + 定向修订 ≤2 轮；T2 真机两态收官 + 缺陷修复回写（2026-09-18 `dedfda064`） |
| [design-md-collection](../../specs/archive/design-md-collection/design.md) | ✅ | design-md 三源设计文档收集（本地目录 + vendored + 复刻 design.extract）S1–S5 全落地（2026-09-18 归档） |
| [design-stage-gates](../../specs/archive/design-stage-gates/design.md) | ✅ | 设计管线五 stage 机械门 + OCR 分层失败语义 WP1–WP4 落地（2026-09-18 归档；门域由 spec-graph-adoption 对账沿用） |
| [prd-theme-layer](../../specs/archive/prd-theme-layer/design.md) | ✅ | PRD 级主题层：主题 CRUD/继承注入/目录分组/两工作台主题条；p-core 真机走查收官（2026-09-18 归档） |
| [prototype-reliability](../../specs/archive/prototype-reliability/design.md) | 🟡 | 原型生成链路根治四工作包：方案定稿未实施即**冻结**（2026-09-19 用户拍板；§4 原型绘制引擎重规划开放决策底座随稿保留） |
...
| [artifact-landing](../../specs/archive/artifact-landing/design.md) | ✅ | 产物落地三链（编辑器二进制兜底预览 / 需求文档幻灯片输出 marp / 落地简报防塌生成器）：三链落地 + 审查修复批 + TD 供应链收尾，tasks 19/19（2026-09-18 归档） |
...
| [prototype-companion](../../specs/archive/design-systems-advance/design.md) | ❌ | 2026-09-02 合并入 design-systems-advance；任务树落点/滚动审计已由 task-tree-hub 实现关闭 |
...
| 功能域                                            | 已集成                                                                                                            | 规划中                                                                                                                                                                           | 核心目标                                                 |
| ------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------- |
| [一、代码智能](#一代码智能)                       | codegraph（in-process SDK）, CRG（风险图谱）, ocr（语义审查）, **serena**（语义操作）, **arch-scan**（架构索引） | **zg 工作区语义检索（zvec-grep，下一版主线 E，P0 门槛）**                                                        | 让 Agent 从"文本级"升级为"语义级"代码操作 + 架构级可视化 |
...
| [六、设计生成](#六设计生成)                       | **DeepDesign `.dd` 管线 + 9 套设计系统 + taste 纪律**, **A2UI 原型模块（Surface 渲染 + update_surface delta + 独立窗口）**, **OpenUI Lang PM-Designer**, dembrandt 品牌摄取（extract/drift） | **html-in-canvas（API 提案，flag 可开）**, dashboard/mobile/poster 模板, Uiverse 组件库                                                                                          | brief→生成→预览→交付 的全流程设计能力                    |
| [七、办公套件](#七办公套件)                       | Bento Slides（单文件 .bento.html）                                                                                  | 文档/表格生成, 办公文档预览面板                                                                                                                                                  | 单文件办公文档（演示文稿/文档/表格）生成与预览           |
| [八、浏览器与联网](#八浏览器与联网)               | browser-skill（真实 Chrome 操控）, **第一方 WebSearch**（DuckDuckGo/Brave/Tavily 适配器）, **内置 WebFetch**（offscreen Chromium + 静态兜底 + SSRF 门）, web-access-strategy（三层联网策略） | obscura                                                                                                                                                                          | 登录态操控 + 大规模抓取 + 深度联网策略                   |
| [九、桌面自动化](#九桌面自动化)                   | —                                                                                                                 | pi-computer-use, ShowUI（VLM 视觉定位）, CLI-Anything, **sim-use（iOS+Android 模拟器，P1）**                                                                                     | 操控无 API 的桌面软件 + 模拟器交互                       |
| [十、引擎演进](#十引擎演进)                       | Plan Mode（force-ask）, UpdatePlan, **MCP SDK 迁移（官方 @modelcontextprotocol/sdk）**, **多端点配置 + createSecondaryClient()**, **崩溃合成收尾（TOOL_NOT_STARTED/OUTCOME_UNKNOWN）**, **两段式 compaction（model-free 预剪）**, **beforeToolExecution 钩子注册表**, **任务树（P0–P2 + P1 收尾：会话徽标/主区 tab/整树归档）** | **长程任务可靠执行（LongHorizon-Harness 原生 MEA）**, A2UI 对话交互层, Prewalk, Subagent                                                                                         | 长程任务可靠执行 + 模型切换 + 子 agent + 交互层升级      |
| [十一、自进化](#十一自进化)                       | skill-writer, skill-digester（静态）, **book-distill（技能从哪来）**, **skill-up CI（S1/S2：评估流水线 + 8 包 14 用例 + 自定义引擎适配器，pin v0.9.0）** | **OpenSpace 闭环「执行→评估→改进」（E1 执行捕获 + E2 改进回路，下一版主线 A）**, Self-Harness/HarnessBank（E3，设计先行）, **JiuwenSwarm 蜂群协作理念**                  | harness 脚手架自改进 + 技能生命周期（生成→评估→改进）    |
| [十二、插件中心](#十二插件中心)                   | **8 插件包分组（skill.plugin.md：browser/code/design/knowledge/memory/meta-skills/vision/work）**, **SkillSpector 安全扫描（68 漏洞模式）**, GitMCP 动态 MCP 前缀, 插件详情双语（builtin.* i18n 全语言） | opencli, 远程源集成（8 Hub）, **Agent Plugins 1.0.0 兼容（双格式）**                                                                                                                                                     | 统一的插件/技能/MCP 管理入口 + 安装安全                  |
| [十三、远程接入](#十三远程接入)                   | —                                                                                                                 | WebSocket 桥 + 静态服务 + 隧道方案（下一版主线 C：M1–M3 + MCP HTTP transport）                                                                                                                                    | 手机/远程浏览器通过蒲公英/ngrok 接入 DeepOrca            |
...
**四层检索分工**（2026-09-02 zg 调研定稿）：bash+rg（已知词/正则穷尽核验）→ codegraph（符号/调用链/影响面）→ serena（实时 LSP 符号定位与语义编辑）→ **zg（模糊意图语义发现，下一版主线 E）**。zg 不做结构关系与符号编辑，是纯内容检索层；中文文档/非代码内容（jieba BM25 + Markdown 标题切分）也归它。第三方独立评测同结论：zg 管入口发现、rg 管精确核验、CodeGraph 管多跳关系。
...
> 实现 spec：[`specs/archive/deep-design/`](../../specs/archive/deep-design/design.md)（✅）· [`specs/archive/a2ui-integration/`](../../specs/archive/a2ui-integration/design.md)（✅）。

### 规划中

| 能力                                     | 项目                                                                             | 集成形态                                                                                                         | 贡献                                                                                                                                                                                                                                                                                                                                                                                 | 优先级               |
| ---------------------------------------- | -------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------- |
| **需求具现化工作台（PM-Design V2）**     | **A2UI + OpenUI + DeepDesign 三管线统一编排**                                    | 左侧 Design 工作区 + `design.materialize` 复合 Action + `pm-analyst` Skill + 设计产物持久化                      | **P0（设计阶段）**：将三管线从"用户三选一"升级为"AI 自动路由"。模拟 PM 完整职责链：需求采集 → 需求分析（pm-analyst 子代理拆解为模块/用户故事/流程）→ 管线路由（交互型→A2UI / 展示型→.dd / 混合型→OpenUI）→ 原型生成 → 预览验证 → 持久化为可管理的设计资产。左侧增加 `design` rail item（位于 Code Review 下方），DesignPanel 提供一键入口 + 产物列表 + 对话迭代闭环。详见 [`specs/pm-design-v2/design.md`](../../specs/archive/design-systems-advance/pm-design-v2-design.md) | P0（设计完成）       |
...
**设计模块进阶方案（design-systems-advance，2026-09-01 立稿；2026-09-02 并入 prototype-companion 为唯一方案）**：对照 open-design / open-pencil / open-codesign / penpot 四项目，指出五类系统性差距（原型交互不可验证 / .dd 单页文档非设计系统 / 质量闸靠提示词 / 产物无留存回退 / 无设计记忆），总纲"契约 + 载体 + 回环"，P0-P3 分期；原型伴随交互（悬浮对话框/侧栏分轨）由其 §5.6 承接。见 [`specs/design-systems-advance/design.md`](../../specs/archive/design-systems-advance/design.md)（🟡 已冻结归档）。
...
- 详见 [`specs/pm-design-v2/design.md`](../../specs/archive/design-systems-advance/pm-design-v2-design.md) · [`tasks.md`](../../specs/archive/design-systems-advance/pm-design-v2-tasks.md)
...
| **多模型家族适配（模型舰队收官）**          | **model-fleet-adaptation**（2026-08-21 立稿，G0+S0 已落地） | 家族注册表唯一事实源（`resolveModelSpec()` 深模块），**原生登记仅保留 deepseek/stepfun**；仅 OpenAI chat-compat 协议。**2026-09-17 吸收实验性 AI SDK 传输线（§七/X 系列：Vercel AI SDK 第二传输通道默认关 + 边界转译不迁移 + models.dev 数据轨）并移入活跃区；同日二次拍板：S1–S4（GLM/Kimi/MiniMax/Qwen）原生登记取消转 X4，后续所有新模型一律依托 X 线接入（R13）**。[`specs/model-fleet-adaptation/design.md`](../../specs/review-ing/model-fleet-adaptation/design.md) | P1     |
...
>
> **首批适配模块与详细设计**：defineAction 首先落地于**代码审查**（CRG + ocr，5 个 action）与**知识索引**（codegraph + openwiki + arch-scan，6 个 action）——两模块当前都患"一个功能碎片化到 MCP/IPC/prompt 三种调用机制"的病。完整原语 API、动作清单、三表面映射、迁移阶段见 [`specs/archive/define-action/design.md`](../../specs/archive/define-action/design.md)。关键难点 `arch-scan.run` 需触发 subagent，为 §十 Subagent（P2）提供首个交汇用例。
