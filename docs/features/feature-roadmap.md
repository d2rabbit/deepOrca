...
| [二、知识中心](#二知识中心)                       | openwiki（Wiki）, **@deeporca/memory**（TDAI L0–L3 持久化记忆）, activity-frames（行为记忆）, **GitMCP 本地化模块**（8 工具） | **doc-wiki 文档知识编译层（下一版主线 D）**, Open Deep Research 理念, **索引与知识增强 K 线（下一版主线 E，2026-08-20 立项）**                          | 项目文档 + 用户资料编译 + 跨会话记忆 + 行为记忆 + 深度研究 |
...
| [十六、能力编排协议](#十六能力编排协议)           | **defineAction 一次定义多表面（28 action 归位：LLM/MCP/IPC 三面 + 统一进度 + M2 spawner）** | **Studio 基座超大版本（下一版主线 B：B1 冷插拔 + B2 热激活/隔离）**，含 H 线借鉴增强 [`specs/studio-base-boost/`](../../specs/studio-base-boost/design.md)（契约测试套/激活门禁/provenance，2026-08-20 立）, OpenWork 双工具 MCP 理念（不做）, 技能/工作流可迁移                                                                                                                                     | 统一能力发现和执行入口（一站化编排层）                   |
...
| **索引与知识增强（K 线，下一版主线 D）** | **PR-AF + FastCode 预研**（思想借鉴，不引入） | 自研：`review.challenge` 对抗证伪 action + session 检索预算记账（详见 [`specs/index-knowledge-boost/design.md`](../../specs/index-knowledge-boost/design.md)） | K1 把流程层"两轮对抗收敛至零"自动化进 review.full；K2 检索预算显式化 + 粒度感知上下文（设计先行）。净室红线：不引入、不 vendor、不拷贝代码与 prompt 原文 | **P1** |
