...
`<project-code>` 由项目根目录路径生成。普通路径会转换为安全的目录名；路径过长时，DeepOrca 会保留项目名的一部分，并追加稳定哈希，以避免存储路径过长。
...
DeepOrca 使用 `file-history/.git` 保存代码快照。这个仓库只作为内部文件历史使用，不是项目仓库本身。
...
快照只覆盖 DeepOrca 已跟踪到的文件；无关文件不会因为 `/undo` 被任意改写。
...
继续会话时，DeepOrca 会读取会话消息文件，过滤已压缩的旧消息，修复未完成的工具调用上下文，并把可用历史转换为模型请求消息。

### 长会话压缩

当会话上下文过长时，DeepOrca 会触发压缩流程：
...
- `file-history/.git` 是 DeepOrca 的内部快照仓库，不应手动修改。
